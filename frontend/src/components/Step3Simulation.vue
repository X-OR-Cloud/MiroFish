...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // Số vòng tối đa được truyền từ Step2
  minutesPerRound: {
    type: Number,
    default: 30 // Mặc định 30 phút mỗi vòng
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
const runStatus = ref({})
const allActions = ref([]) // Tất cả các hành động (tích lũy tăng dần)
const actionIds = ref(new Set()) // Tập hợp ID hành động để khử trùng
const scrollContainer = ref(null)

// Computed
// Hiển thị các hành động theo thứ tự thời gian (mới nhất ở cuối, tức phía dưới)
const chronologicalActions = computed(() => {
  return allActions.value
})

// Số lượng hành động ở mỗi nền tảng
...
// Định dạng thời gian trôi qua của mô phỏng (tính toán dựa trên vòng và số phút mỗi vòng)
...
// Thời gian trôi qua của nền tảng Twitter
const twitterElapsedTime = computed(() => {
  return formatElapsedTime(runStatus.value.twitter_current_round || 0)
})

// Thời gian trôi qua của nền tảng Reddit
...
// Đặt lại tất cả các trạng thái (dùng để khởi động lại mô phỏng)
const resetAllState = () => {
  phase.value = 0
  runStatus.value = {}
  allActions.value = []
  ...
  stopPolling()  // Dừng bất kỳ lấy dữ liệu nào có thể tồn tại trước đó
}

// Khởi động mô phỏng
const doStartSimulation = async () => {
  if (!props.simulationId) {
    addLog('Lỗi: Thiếu simulationId')
    return
  }

  // Đặt lại tất cả trạng thái trước, đảm bảo không bị ảnh hưởng bởi mô phỏng trước đó
  resetAllState()

  isStarting.value = true
  startError.value = null
  addLog('Đang khởi động mô phỏng song song hai nền tảng...')
  ...
  try {
    const params = {
      ...
      platform: 'parallel',
      force: true,  // Buộc khởi động lại
      enable_graph_memory_update: true  // Kích hoạt cập nhật đồ thị động
    }

    if (props.maxRounds) {
      params.max_rounds = props.maxRounds
      addLog(`Đặt số vòng mô phỏng tối đa: ${props.maxRounds}`)
    }

    addLog('Đã kích hoạt chế độ cập nhật đồ thị động')

    const res = await startSimulation(params)

    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ Đã xóa nhật ký mô phỏng cũ, khởi động lại mô phỏng')
      }
      addLog('✓ Khởi động động cơ mô phỏng thành công')
      ...
      startError.value = res.error || 'Khởi động thất bại'
      addLog(`✗ Khởi động thất bại: ${res.error || 'Lỗi không xác định'}`)
      emit('update-status', 'error')
    }
  } catch (err) {
    startError.value = err.message
    addLog(`✗ Lỗi khởi động: ${err.message}`)
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
const handleStopSimulation = async () => {
  ...
  addLog('Đang dừng mô phỏng...')

  try {
    const res = await stopSimulation({ simulation_id: props.simulationId })

    if (res.success) {
      addLog('✓ Mô phỏng đã dừng')
      phase.value = 2
      stopPolling()
      emit('update-status', 'completed')
    } else {
      addLog(`Dừng thất bại: ${res.error || 'Lỗi không xác định'}`)
    }
  } catch (err) {
    addLog(`Lỗi dừng: ${err.message}`)
  } finally {
    isStopping.value = false
  }
}
...
// Theo dõi vòng cuối cùng ở mỗi nền tảng, được sử dụng để phát hiện thay đổi và xuất nhật ký
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Phát hiện các thay đổi vòng ở mỗi nền tảng và xuất nhật ký
      ...
      // Phát hiện liệu mô phỏng đã hoàn thành (thông qua runner_status hoặc trạng thái hoàn thành nền tảng)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'

      // Kiểm tra bổ sung: nếu backend chưa kịp cập nhật runner_status nhưng nền tảng đã báo hoàn thành
      // Phát hiện thông qua trạng thái twitter_completed và reddit_completed
      const platformsCompleted = checkPlatformsCompleted(data)

      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ Phát hiện tất cả nền tảng mô phỏng đã kết thúc')
        }
        addLog('✓ Mô phỏng đã hoàn thành')
        phase.value = 2
        stopPolling()
        emit('update-status', 'completed')
      }
    ...
    console.warn('Lấy trạng thái chạy thất bại:', err)
  }
}

// Kiểm tra xem tất cả các nền tảng được kích hoạt đã hoàn thành chưa
const checkPlatformsCompleted = (data) => {
  // Nếu không có dữ liệu nền tảng, trả về false
  if (!data) return false

  // Kiểm tra trạng thái hoàn thành ở mỗi nền tảng
  const twitterCompleted = data.twitter_completed === true
  const redditCompleted = data.reddit_completed === true

  // Nếu ít nhất một nền tảng hoàn thành, kiểm tra xem tất cả nền tảng được kích hoạt có hoàn thành không
  // Xác định xem nền tảng có được kích hoạt không thông qua actions_count (nếu count > 0 hoặc running từng là true)
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  const redditEnabled = (data.reddit_actions_count > 0) || data.reddit_running || redditCompleted

  // Nếu không có nền tảng nào được kích hoạt, trả về false
  if (!twitterEnabled && !redditEnabled) return false

  // Kiểm tra xem tất cả các nền tảng được kích hoạt có hoàn thành không
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // Sử dụng all_actions để lấy danh sách hành động hoàn chỉnh
      const serverActions = res.data.all_actions || []

      // Thêm hành động mới tăng dần (khử trùng)
      ...
      // Không tự động cuộn, cho phép người dùng tự do xem dòng thời gian
      // Các hành động mới sẽ được thêm vào phía dưới
    }
  } catch (err) {
    console.warn('Lấy trạng thái chi tiết thất bại:', err)
  }
}
