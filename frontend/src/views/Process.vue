...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'No pending files to upload. Please return to home page and try again'
    loading.value = false
    return
  }
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      // Continue polling in-progress tasks
      ...
      error.value = response.error || 'Project loading failed'
    }
  } catch (err) {
    console.error('Load project error:', err)
    error.value = 'Project loading failed: ' + (err.message || 'unknown error')
  } finally {
    loading.value = false
  }
}
  ...
  try {
    ...
    if (projectResponse.success && projectResponse.data.graph_id) {
      ...
      if (graphResponse.success && graphResponse.data) {
        ...
        // Update rendering if data changes
...
/* Hiển thị quan hệ chi tiết cạnh */
.edge-relation {
  display: flex;
  align-items: center;
  flex-wrap: wrap;
