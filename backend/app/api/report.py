"""
Report API routes
Provide interfaces for simulation report generation, retrieval, and conversation
"""
...
def generate_report():
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "Missing graph ID, please make sure the graph is built"
            }), 400
        ...
        return jsonify({
            ...
            "data": {
                ...
                "task_id": task_id,
                "status": "generating",
                "message": "Report generation task started, please query progress through /api/report/generate/status",
...
def get_generate_status():
    ...
    try:
        ...
        # textsimulation_id，textChecktextCompletetextReport
        if simulation_id:
            existing_report = ReportManager.get_report_by_simulation(simulation_id)
            if existing_report and existing_report.status == ReportStatus.COMPLETED:
                return jsonify({
...
def get_report_progress(report_id: str):
    ...
    try:
        ...
        if not progress:
            return jsonify({
                "success": False,
                "error": f"ReporttextInfotext: {report_id}"
            }), 404
...
def get_report_sections(report_id: str):
    """
    Get list of generated sections (section output)
...
def get_agent_log(report_id: str):
    ...
    Get every step action in the report generation process in real time, including:
...
def get_console_log(report_id: str):
    ...
    Get console output in real time during report generation (INFO, WARNING, etc.),
    This is different from the structured JSON logs returned by the agent-log interface,
    It is a plain text format console style log.
...
def search_graph_tool():
    ...
    try:
        ...
        if not graph_id or not query:
            return jsonify({
                "success": False,
                "error": "text graph_id text query"
            }), 400
