"""Zep Graph textReadTool。

Zep text node/edge ListInterfacetext UUID cursor text，
textEncapsulationtext（textRetry），textReturnstextList。
"""
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """textRequest，FailedtextRetry。textRetryNetwork/IOClasstextError。"""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """textGetGraphNode，textReturns max_items text（text 2000）。textRequesttextRetry。"""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """textGetGraphtextEdge，ReturnstextList。textRequesttextRetry。"""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
