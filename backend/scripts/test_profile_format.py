"""
textProfileFormatGeneratetextOASIStext
Validate：
1. Twitter ProfileGenerateCSVFormat
2. Reddit ProfileGenerateJSONtextFormat
"""
...
def test_profile_formats():
    """textProfileFormat"""
    print("=" * 60)
    print("OASIS ProfileFormattext")
    print("=" * 60)
    
    # CreatetextProfileData
    test_profiles = [
        OasisAgentProfile(
            user_id=0,
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # textTwitter CSVFormat
        print("\n1. textTwitter Profile (CSVFormat)")
        print("-" * 40)
        ...
        print(f"   text: {len(rows)}")
        print(f"   Tabletext: {list(rows[0].keys())}")
        print(f"\n   ExampleData (text1text):")
        ...
        print(f"   [Information] textField: {present_optional}")
...
def show_expected_formats():
    """ShowOASIStextFormat"""
    print("\n" + "=" * 60)
    print("OASIS textProfileFormattext")
    print("=" * 60)
...
1,user1,User One,Tech enthusiast and coffee lover.,200,250,1000,2023-01-02"""
    ...
    print("\n2. Reddit Profile (JSONtextFormat)")
    print("-" * 40)
    reddit_example = [
        {
